        ...
        implements Accumulator
{
    ...
    {
        F64Vector stateValues = (F64Vector) state.values();
        ...
        for (int position : mask) {
            if (!inputNulls.value(position)) {
                if (stateNulls.values()[group]) {
                    ...
                    stateValues.values()[group] = Math.max(stateValues.values()[group], inputValues.value(position));
                }
            }
        }
    }
