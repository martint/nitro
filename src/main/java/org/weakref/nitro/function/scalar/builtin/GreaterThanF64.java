...
{
    private final Allocator.Context allocationContext = new Allocator.Context("GreaterThanF64");
    ...
    public Streams apply(List<Streams> inputs, Mask mask, Set<Stream> requestedStreams, Streams output, PrimitiveExecutionContext context)
    {
        ...
        if (requestedStreams.contains(Stream.VALUES)) {
            BooleanVector fast = NullFreeScalarKernels.compareDouble(NullFreeScalarKernels.GREATER_THAN, inputs.get(0).values(), inputs.get(1).values(), inputs.get(0).getOrNull(Stream.NULLS), inputs.get(1).getOrNull(Stream.NULLS), mask, output != null ? output.getOrNull(Stream.VALUES) : null, context.allocator(), allocationContext);
            if (fast != null) {
