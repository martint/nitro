...
/**
 * Provider-side scaffold for null-propagating binary F64 projection programs.
 */
abstract class BinaryF64ProjectionOptimization
        implements ProjectionCodeProvider
{
    @Override
    public final Optional<ProjectionProgram> generate(ProjectionCodeBuilder builder, List<ProjectionArgument> arguments)
    {
        if (arguments.size() != 2) {
        ...
        return Optional.of(builder.program(
                ...
                value(builder, left, right),
                builder.or(builder.isNull(0), builder.isNull(1))));
    }

    protected abstract ProjectionCodeBuilder.Value value(
