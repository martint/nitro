...
{
    private static final Allocator.Context ALLOCATION_CONTEXT = new Allocator.Context("CastUtf8ToI64");
    ...
    private static void apply(Vector values, BooleanVector inputNulls, Mask mask, I64Vector output)
    {
        ...
        if (mask.all()) {
            for (int position = 0; position < mask.size(); position++) {
                outputValues[position] = isNull(inputNulls, position) ? 0 : parse(values, position);
            }
            return;
        }
