        ...
        implements PrimitiveFunction
{
    ...
    public Streams apply(List<Streams> inputs, Mask mask, Set<Stream> requestedStreams, Streams output, PrimitiveExecutionContext context)
    {
        ...
        if (requestedStreams.contains(Stream.VALUES)) {
            for (int position : mask) {
                Vector selected = conditionValue(condition, position) ? trueValues : falseValues;
        ...
        if (requestedStreams.contains(Stream.VALUES)) {
            ...
            applyValues(condition, trueValues, falseValues, trueNulls, falseNulls, mask, outputValues, outputNulls);
            result = result.with(Stream.VALUES, outputValues);
            return result;
        }

        if (outputNulls != null) {
            applyNulls(condition, trueNulls, falseNulls, mask, outputNulls);
        }
        return result;
    }

    private static void applyValues(Vector condition, Vector trueValues, Vector falseValues, BooleanVector trueNulls, BooleanVector falseNulls, Mask mask, BinaryVector outputValues, BooleanVector outputNulls)
    {
        for (int position : mask) {
            boolean takeTrue = conditionValue(condition, position);
    ...
    private static void applyNulls(Vector condition, BooleanVector trueNulls, BooleanVector falseNulls, Mask mask, BooleanVector outputNulls)
    {
    ...
    private static boolean conditionValue(Vector values, int position)
    {
        return switch (values) {
            case BooleanVector vector -> vector.values()[position];
            case DictionaryVector vector -> conditionValue(vector.values(), vector.ids()[position]);
            case RleVector vector -> conditionValue(vector.values(), vector.runIndex(position));
