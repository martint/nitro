...
/** Rounds an F64 value to the nearest integer, with exact halves rounded away from zero. */
@ScalarFunction(name = "round_f64")
public final class RoundF64
        implements PrimitiveFunction
{
    ...
    {
        checkArgument(inputs.size() == 1, "Unexpected argument count for round_f64");
        ...
        Allocator.Context allocationContext = context.allocationContext("RoundF64");
        VectorAccess.DoubleValues inputValues = VectorAccess.doubleValues(inputs.getFirst().values());
        ...
        for (int position : mask) {
            double value = inputValues.value(position);
            outputValues[position] = Math.copySign(Math.floor(Math.abs(value) + 0.5), value);
        }
