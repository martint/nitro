...
@ScalarFunction(name = "divide_f64")
public final class DivideF64
        implements PrimitiveFunction
{
    ...
    {
        checkArgument(inputs.size() == 2, "Unexpected argument count for divide_f64");
        ...
        Allocator.Context allocationContext = context.allocationContext("DivideF64");
        VectorAccess.DoubleValues numeratorValues = VectorAccess.doubleValues(inputs.get(0).values());
        VectorAccess.DoubleValues denominatorValues = VectorAccess.doubleValues(inputs.get(1).values());
        ...
        if (requestNulls) {
            ...
            for (int position : mask) {
                double denominator = denominatorValues.value(position);
                nullValues[position] = numeratorNulls.value(position) || denominatorNulls.value(position) || denominator == 0.0;
            }
        ...
        for (int position : mask) {
            double denominator = denominatorValues.value(position);
            outputValues[position] = denominator == 0.0 ? 0.0 : numeratorValues.value(position) / denominator;
        }
