...
public final class ParquetCompressionUtils
{
    ...
    private static final ThreadLocal<SnappyDecompressor> SNAPPY_DECOMPRESSOR = ThreadLocal.withInitial(SnappyDecompressor::create);
    private static final ThreadLocal<ZstdDecompressor> ZSTD_DECOMPRESSOR = ThreadLocal.withInitial(ZstdDecompressor::create);
    private static final ThreadLocal<Decompressor> LZ4_DECOMPRESSOR = ThreadLocal.withInitial(Lz4Decompressor::create);
    private static final ThreadLocal<Decompressor> LZ4_RAW_DECOMPRESSOR = ThreadLocal.withInitial(Lz4JavaDecompressor::new);
    private static final ThreadLocal<Decompressor> LZO_DECOMPRESSOR = ThreadLocal.withInitial(LzoDecompressor::new);
            ...
            throws IOException
    {
        ...
        return switch (codec) {
            ...
            case LZ4_RAW -> decompressLz4Raw(input, uncompressedSize);
            case ZSTD -> decompressZstd(input, uncompressedSize);
            case BROTLI -> throw new ParquetCorruptionException(dataSourceId, "Codec not supported in Parquet: %s", codec);
        };
    }
    ...
    private static Slice decompressSnappy(Slice input, int uncompressedSize)
    {
        ...
        int actualUncompressedSize = decompress(SNAPPY_DECOMPRESSOR.get(), input, 0, input.length(), buffer, 0);
    ...
    private static Slice decompressZstd(Slice input, int uncompressedSize)
    {
        ...
        decompress(ZSTD_DECOMPRESSOR.get(), input, 0, input.length(), buffer, 0);
    ...
    {
        return decompressFramed(LZ4_DECOMPRESSOR.get(), input, uncompressedSize);
    }

    private static Slice decompressLz4Raw(Slice input, int uncompressedSize)
    {
        byte[] buffer = new byte[uncompressedSize + SIZE_OF_LONG];
        int actualUncompressedSize = decompress(LZ4_RAW_DECOMPRESSOR.get(), input, 0, input.length(), buffer, 0);
        if (actualUncompressedSize != uncompressedSize) {
            throw new IllegalArgumentException(format("Invalid uncompressedSize for LZ4_RAW input. Expected %s, actual: %s", uncompressedSize, actualUncompressedSize));
        }
        return wrappedBuffer(buffer, 0, uncompressedSize);
    }
    ...
    {
        return decompressFramed(LZO_DECOMPRESSOR.get(), input, uncompressedSize);
    }
