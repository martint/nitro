...
@ScalarFunction(name = "least_i64")
public final class LeastI64
        implements PrimitiveFunction
{
    ...
    {
        checkArgument(!inputs.isEmpty(), "least_i64 requires at least one argument");
        ...
        Allocator.Context allocationContext = context.allocationContext("LeastI64");
        ...
        for (int position : mask) {
            long least = arguments.getFirst().value(position);
            for (int argument = 1; argument < arguments.size(); argument++) {
                least = Math.min(least, arguments.get(argument).value(position));
            }
            outputValues[position] = least;
        }
