        ...
        implements PrimitiveFunction
{
    ...
    {
        checkArgument(!inputs.isEmpty(), "greatest_f64 requires at least one argument");
        ...
        Allocator.Context allocationContext = context.allocationContext("GreatestF64");
        ...
        F64Vector values = context.allocator().allocateOrGrow(
                ...
                requiredLength,
                F64Vector::new);
        List<VectorAccess.DoubleValues> arguments = inputs.stream()
                .map(Streams::values)
                .map(VectorAccess::doubleValues)
                .toList();
        double[] outputValues = values.values();
        for (int position : mask) {
            double greatest = arguments.getFirst().value(position);
