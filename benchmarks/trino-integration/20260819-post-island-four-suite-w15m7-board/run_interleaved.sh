...
run_one()
{
    ...
    if [[ -s "$timings" ]]; then
        printf 'SKIP %s %s %s (already measured)\n' "$engine" "$suite" "$query"
        return
    fi
...
run_engine_coverage()
{
    ...
    for query_number in $(seq -w 1 27); do
        ...
        if (( ordinal % 2 == 0 )); then
            run_one nitro-c engine-coverage "$query" io.trino.tests.benchmark.BenchmarkHiveEngineCoverage /root/data/engine-coverage-v3 engine-coverage
            run_one cork engine-coverage "$query" io.trino.tests.benchmark.BenchmarkHiveEngineCoverage /root/data/engine-coverage-v3 engine-coverage
        else
            run_one cork engine-coverage "$query" io.trino.tests.benchmark.BenchmarkHiveEngineCoverage /root/data/engine-coverage-v3 engine-coverage
            run_one nitro-c engine-coverage "$query" io.trino.tests.benchmark.BenchmarkHiveEngineCoverage /root/data/engine-coverage-v3 engine-coverage
        fi
