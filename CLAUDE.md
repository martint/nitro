...
optionally publish exact `GroupedAggregationUpdateTarget` method handles over an ordered tuple of primitive
contributions; generated engine code flattens the physical inputs, applies the combined null convention, and
constant-links those targets while treating provider state as opaque rather than referencing a builtin state class.
...
invokes the single update once per logical occurrence. Do not admit a multi-input encoded-domain shortcut without
proving that every value and null mapping shares the same physical domain.
